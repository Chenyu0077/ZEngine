...
namespace ZEngine.Manager.Network
{
    ...
    public class TcpChannel : NetworkChannelBase
    {
        ...
        /// 重连最大等待间隔（秒）
        /// </summary>
        public float MaxReconnectInterval { get; set; } = 60f;
        ...
        public override void Connect(string host, int port)
        {
            ...
            {
                ZEngineLog.Warning("[TcpChannel] 已经在连接或已连接状态，请先断开");
                return;
            }
        ...
        public override void Disconnect()
        {
            ...
            {
                ZEngineLog.Warning($"[TcpChannel] 断开连接异常: {ex.Message}");
            }
            ...
            ZEngineLog.Log("[TcpChannel] 已断开连接");
        }
        ...
        public override void Send(int messageId, IMessage message)
        {
            if (!IsConnected)
            {
                ZEngineLog.Warning("[TcpChannel] 未连接，无法发送消息");
                return;
            }
        ...
        public void SendHeartbeat()
        {
            if (!IsConnected)
                return;
            EnqueueHeartbeatPacket();
            _lastHeartbeatSendTime = 0;
        }

        /// <summary>
        /// 内部入队心跳包，不修改计时器（由 UpdateHeartbeat 控制精度）
        /// </summary>
        private void EnqueueHeartbeatPacket()
        {
        ...
        private void UpdateHeartbeat(float elapseSeconds)
        {
            ...
            // 使用 -= 而非 = 0，避免帧峰值导致的计时器漂移
            if (_lastHeartbeatSendTime >= HeartbeatInterval)
            {
                _lastHeartbeatSendTime -= HeartbeatInterval;
                EnqueueHeartbeatPacket();
            }
            ...
            if (_lastHeartbeatReceiveTime >= HeartbeatTimeout)
            {
                ...
                ZEngineLog.Warning($"[TcpChannel] 心跳超时 ({_heartbeatTimeoutCount}/{MaxHeartbeatTimeoutCount})");

                if (_heartbeatTimeoutCount >= MaxHeartbeatTimeoutCount)
                {
                    ZEngineLog.Error("[TcpChannel] 心跳超时次数过多，断开连接");
        ...
        private void StartReconnect()
        {
            ...
            {
                ZEngineLog.Error($"[TcpChannel] 重连次数已达上限 ({MaxReconnectCount})");
                _isReconnecting = false;
                OnReconnectFailed();
                return;
            }
            ...
            // 指数退避：base * 2^(n-1)，上限 MaxReconnectInterval，加 ±20% 随机抖动
            float backoff = ReconnectInterval * (float)Math.Pow(2, _reconnectCount - 1);
            backoff = Math.Min(backoff, MaxReconnectInterval);
            backoff *= UnityEngine.Random.Range(0.8f, 1.2f);
            _currentReconnectInterval = backoff;

            ZEngineLog.Log($"[TcpChannel] 准备重连 ({_reconnectCount}/{MaxReconnectCount})，等待 {_currentReconnectInterval:F1} 秒...");
        ...
        private void UpdateReconnect(float elapseSeconds)
        {
            ...
            if (_reconnectWaitTime >= _currentReconnectInterval)
            {
                _reconnectWaitTime = 0;
                DoConnect();
            }
        ...
        private bool ExpandBuffer(int requiredLength)
        {
            ...
            {
                ZEngineLog.Error($"[TcpChannel] 数据超过最大缓冲区限制: {requiredLength} > {MaxBufferSize}");
                return false;
            }
            ...
            ZEngineLog.Log($"[TcpChannel] 缓冲区扩容: {_dataBuffer.Length} -> {newSize} bytes");
            return true;
        }
        ...
        private void ShrinkBuffer()
        {
                ...
                _dataLength < _dataBuffer.Length / 4)
            {
                ...
                if (newSize >= _dataLength)
                {
                    ...
                    ZEngineLog.Log($"[TcpChannel] 缓冲区缩容: {_dataBuffer.Length} -> {newSize} bytes");
                    _dataBuffer = newBuffer;
                }
            }
        }
            ...
            while (_isRunning)
            {
                ...
                while (_isRunning)
                {
                    ...
                    if (data != null)
                    {
                        ...
                        {
                            ZEngineLog.Error($"[TcpChannel] 发送失败: {ex.Message}");
        ...
        private void ReceiveLoop()
        {
            while (_isRunning && _socket != null && _socket.Connected)
            {
                ...
                catch (SocketException ex)
                {
                    if (_isRunning)
                    {
                        ZEngineLog.Error($"[TcpChannel] 接收异常: {ex.Message}");
                        HandleDisconnect(DisconnectReason.NetworkError);
                    }
                    return;
                }
        ...
        private void ProcessReceivedData()
        {
            while (_dataLength >= HeaderSize)
            {
                ...
                {
                    ZEngineLog.Error($"[TcpChannel] 无效的消息长度: {bodyLength}");
                    HandleDisconnect(DisconnectReason.NetworkError);
                    return;
                }
                ...
                {
                    ZEngineLog.Error("[TcpChannel] 解码失败");
                    HandleDisconnect(DisconnectReason.NetworkError);
                    return;
                }
        ...
        private void StopThreads()
        {
            ...
            if (_sendThread != null)
            {
                ...
                {
                    ZEngineLog.Warning("[TcpChannel] 发送线程未能在超时时间内结束");
                }
                _sendThread = null;
            }
            ...
            if (_receiveThread != null)
            {
                ...
                {
                    ZEngineLog.Warning("[TcpChannel] 接收线程未能在超时时间内结束");
                }
                _receiveThread = null;
            }
        }
        ...
        private void DoConnect()
        {
            ...
            try
            {
                ...
                if (success && _socket.Connected)
                {
                    ...
                    if (_isReconnecting)
                    {
                        _isReconnecting = false;
                        _reconnectCount = 0;
                        ZEngineLog.Log($"[TcpChannel] 重连成功: {Host}:{Port}");
                        OnConnected();
                        _networkManager?.OnChannelReconnected(this);
                    }
                    else
                    {
                        ZEngineLog.Log($"[TcpChannel] 连接成功: {Host}:{Port}");
                        OnConnected();
                    }
                }
                else
                {
                    ...
                    if (_isReconnecting)
                    {
                        ZEngineLog.Warning("[TcpChannel] 重连失败，继续尝试...");
                        StartReconnect();
                    }
                    else
                    {
            ...
            catch (Exception ex)
            {
                ...
                if (_isReconnecting)
                {
                    ZEngineLog.Warning($"[TcpChannel] 重连异常: {ex.Message}，继续尝试...");
                    StartReconnect();
                }
                else
                {
                    OnConnectFailed(ex.Message);
                    ZEngineLog.Error($"[TcpChannel] 连接失败: {ex.Message}");
                }
            }
        }
