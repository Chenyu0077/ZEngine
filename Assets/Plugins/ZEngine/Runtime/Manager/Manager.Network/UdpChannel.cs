...
namespace ZEngine.Manager.Network
{
    ...
    public class UdpChannel : NetworkChannelBase, INetEventListener
    {
        ...
        /// 连接密钥
        /// </summary>
        private string _connectionKey = "ZEngine";
        ...
        public override void Connect(string host, int port)
        {
            ...
            {
                ZEngineLog.Warning("[UdpChannel] 已经在连接或已连接状态，请先断开");
                return;
            }
        ...
        private void DoConnect()
        {
            ...
            try
            {
                ...
                {
                    AutoRecycle = true,         // 自动回收 NetPacketReader
                    EnableStatistics = false,   // 禁用统计（减少开销）
                    UnconnectedMessagesEnabled = false,
                    NatPunchEnabled = false,
                    DisconnectTimeout = 10000, // 10秒超时
                    PingInterval = 1000,       // 1秒ping一次
                    ReconnectDelay = 500,       // 重连延迟
                    MaxConnectAttempts = 5      // 最大连接尝试次数
                };
                ...
                ZEngineLog.Log($"[UdpChannel] 正在连接: {Host}:{Port}");
            }
            catch (Exception ex)
            {
                _netManager?.Stop();
                ...
                ZEngineLog.Error($"[UdpChannel] 连接失败: {ex.Message}");
            }
        }
        ...
        private void HandleConnectFailed(string message)
        {
            if (_isReconnecting)
            {
                ZEngineLog.Warning("[UdpChannel] 重连失败，继续尝试...");
        ...
        public override void Disconnect()
        {
            ...
            {
                ZEngineLog.Warning($"[UdpChannel] 断开连接异常: {ex.Message}");
            }

            OnDisconnected(DisconnectReason.Manual);
            ZEngineLog.Log("[UdpChannel] 已断开连接");
        }
        ...
        public override void Send(int messageId, IMessage message)
        {
            ...
            {
                ZEngineLog.Warning("[UdpChannel] 未连接，无法发送消息");
                return;
            }
        ...
        private void StartReconnect()
        {
            ...
            {
                ZEngineLog.Error($"[UdpChannel] 重连次数已达上限 ({MaxReconnectCount})");
                _isReconnecting = false;
                OnReconnectFailed();
                return;
            }
            ...
            ZEngineLog.Log($"[UdpChannel] 准备重连 ({_reconnectCount}/{MaxReconnectCount})，等待 {ReconnectInterval} 秒...");
        ...
        private void SendData(byte[] data, DeliveryMethod deliveryMethod)
        {
            ...
            {
                ZEngineLog.Error($"[UdpChannel] 发送失败: {ex.Message}");
            }
        }
        ...
        public void OnPeerConnected(NetPeer peer)
        {
            ...
            if (_isReconnecting)
            {
                _isReconnecting = false;
                _reconnectCount = 0;
                ZEngineLog.Log($"[UdpChannel] 重连成功: {peer.Address}");
                _serverPeer = peer;
                OnConnected();
            ...
            {
                ZEngineLog.Log($"[UdpChannel] 连接成功: {peer.Address}");
                _serverPeer = peer;
                OnConnected();
            }
        }
        ...
        {
            ZEngineLog.Log($"[UdpChannel] 断开连接: {peer.Address}, 原因: {disconnectInfo.Reason}");
            _serverPeer = null;
        ...
        {
            ZEngineLog.Error($"[UdpChannel] 网络错误: {endPoint}, {socketError}");
        ...
        public void OnNetworkReceive(NetPeer peer, NetPacketReader reader, byte channel, DeliveryMethod deliveryMethod)
        {
            try
            {
                ...
                {
                    ZEngineLog.Error("[UdpChannel] 解码失败");
                }
            }
            catch (Exception ex)
            {
                ZEngineLog.Error($"[UdpChannel] 处理接收数据异常: {ex.Message}");
            }
        }
