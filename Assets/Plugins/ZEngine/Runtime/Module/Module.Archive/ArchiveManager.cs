    ...
    public class ArchiveManager : ManagerSingleton<ArchiveManager>, IManager
    {
        ...
        public void OnInit(object param)
        {
            ...
            if (ZEngineMain.Contains(typeof(LogManager)) == false)
                throw new Exception($"{nameof(ArchiveManager)}依赖于{nameof(LogManager)}");
            ...
            LogManager.Instance.Info($"本地存档地址：{GetSlotInfosPath()}");
            ...
            else
            {
                try
                {
                    ...
                    LogManager.Instance.Info($"存档初始化成功");
                }
                catch (Exception e)
                {
                    LogManager.Instance.Error($"加载存档信息列表失败: {e}");
                    slotInfos = new List<SlotInfo>();
                }
            }
        }
        ...
        /// <summary>
        /// 同步加载存档，返回加载到的槽位对象（文件不存在时返回 null）
        /// </summary>
        public T LoadSync<T>(string fileName) where T : ArchiveSlotBase, new()
        {
            ...
            // 缓存到内存字典，使 GetSlot 可以取到
            slotDic[slot.ID] = slot;
            return slot;
        }
