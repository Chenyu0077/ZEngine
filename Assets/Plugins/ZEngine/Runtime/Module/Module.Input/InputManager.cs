...
namespace ZEngine.Module.Input
{
    ...
    public class InputManager
    {
        ...
        {
            if (AllKeyboards.Count > 0)
            {
                return InputDeviceType.Keyboard;
            }
            else if(AllGamepads.Count > 0)
            {
                return InputDeviceType.Gamepad;
            }
            else if(AllPointers.Count > 0)
            {
        ...
        public static void GamepadVibrate(Gamepad gamepad, float low, float hight, float time)
        {
            if (gamepad == null || !gamepad.enabled) return;
            // 设置震动速度
            gamepad.SetMotorSpeeds(low, hight);
            gamepad.ResumeHaptics();
            TimerManager.Instance.CreateOnceTimer(() =>
            {
